#!/usr/bin/env bash
# Container-side launcher for minimal TMax PI + Slime GRPO training.
...
run_id="${run_id:-tmax_pi_minimal_$(date -u +%Y%m%dT%H%M%SZ)}"
run_label="${run_label:-tmax-minimal}"
dry_run="${dry_run:-0}"
...
apptainer_image_dir="$(abs_path "${apptainer_image_dir:-${tmax_image_dir}}")"
...
job_cache_root="${job_cache_root:-/tmp/tmax-minimal-${agent_label}-${SLURM_JOB_ID:-manual}-${run_id}}"
...
polar_early_stop_grace_sessions="${polar_early_stop_grace_sessions:-2}"
...
if [ "${qwen_gdn_backend}" = "flashqla" ]; then
    ...
    export PYTHONPATH="${flashqla_override_dir}:${megatron_dir}:${slime_dir}:${runtime_project_src}:${PYTHONPATH:-}"
elif [ -n "${fla_override_dir}" ]; then
    [ -d "${fla_override_dir}/fla" ] || die "FLA runtime override missing: ${fla_override_dir}"
    export PYTHONPATH="${fla_override_dir}:${megatron_dir}:${slime_dir}:${runtime_project_src}:${PYTHONPATH:-}"
else
    ...
    export PYTHONPATH="${megatron_dir}:${slime_dir}:${runtime_project_src}:${PYTHONPATH:-}"
fi
...
prepare_prompt_data_and_sifs() {
    "${python_bin}" - "${full_prompt_data}" "${prompt_data}" "${smoke_rows}" <<'PY'
import json
import sys
from pathlib import Path
...
if not rows:
    raise SystemExit(f"no TMax prompt rows selected from {full_prompt}")

missing = []
for row in rows:
    metadata = row.get("metadata") or {}
    task_name = metadata.get("task_name")
    sif_path = Path(str(metadata.get("sif_path", ""))).expanduser()
    tests_dir = Path(str(metadata.get("tests_dir", ""))).expanduser()
    if not task_name:
        missing.append("row without metadata.task_name")
    if not sif_path.is_file() or sif_path.stat().st_size <= 0:
        missing.append(f"{task_name}: missing SIF {sif_path}")
    if not tests_dir.is_dir():
        missing.append(f"{task_name}: missing tests_dir {tests_dir}")
if missing:
    raise SystemExit("invalid TMax rows:\n  " + "\n  ".join(missing[:20]))

prompt_out.parent.mkdir(parents=True, exist_ok=True)
prompt_out.write_text("\n".join(json.dumps(row, ensure_ascii=True) for row in rows) + "\n")
print(f"Prompt data: {prompt_out} ({len(rows)} validated TMax row(s))")
PY
}

prepare_agent_cli() {
    if [ "${force_agent_cli}" != "1" ] && [ -x "${agent_cli_dir}/bin/pi" ] && [ -x "${agent_cli_dir}/bin/node" ]; then
        if PATH="${agent_cli_dir}/bin:${PATH}" "${agent_cli_dir}/bin/pi" --version >/dev/null 2>&1; then
            log "Agent CLI exists and passed PI dependency preflight: ${agent_cli_dir}"
            return
        fi
        log "Agent CLI binaries exist but PI dependency preflight failed; rebuilding: ${agent_cli_dir}"
        force_agent_cli=1
    fi
...
render_runtime_configs() {
    ...
    export POLAR_MAX_REPLACEMENT_GROUPS=${polar_max_replacement_groups}
    export POLAR_FULLY_ASYNC="${polar_fully_async}"
...
config["polar_max_replacement_groups"] = int(os.environ["POLAR_MAX_REPLACEMENT_GROUPS"])
config["polar_fully_async"] = str(os.environ.get("POLAR_FULLY_ASYNC", "")).strip().lower() in {"1", "true", "yes", "on"}
config["polar_early_stop_grace_sessions"] = int(os.environ["POLAR_EARLY_STOP_GRACE_SESSIONS"])
max_trajectory_tokens = os.environ.get("POLAR_MAX_TRAJECTORY_TOKENS", "").strip()
if max_trajectory_tokens:
    config["polar_max_trajectory_tokens"] = int(max_trajectory_tokens)

task = config.setdefault("polar_task_template", {})
if os.environ.get("POLAR_TASK_TIMEOUT_FROM_METADATA", "0") == "1":
...
prepare_reference_checkpoint_compat() {
    [ "${reference_recipe}" = "1" ] || return 0
    local source_ref="${ref_load}"
    local compat_ref="${reference_ref_load:-${script_dir}/tmp/checkpoint_compat/Qwen3.5-4B_torch_dist_mcore}"
    local source_release="${source_ref}/release"
    local compat_release="${compat_ref}/release"
    [ -f "${source_release}/common.pt" ] || die "reference checkpoint common state missing: ${source_release}/common.pt"
    mkdir -p "${compat_release}"
    local name
    for name in __0_0.distcp .metadata metadata.json; do
        [ -f "${source_release}/${name}" ] || die "reference checkpoint file missing: ${source_release}/${name}"
        ln -sfn "${source_release}/${name}" "${compat_release}/${name}"
    done
    "${python_bin}" - "${source_release}/common.pt" "${compat_release}/common.pt" <<'PY'
import os
import sys
import torch
...
args = state.get("args")
if args is None:
    raise SystemExit(f"checkpoint common state has no args: {source}")
setattr(args, "async_strategy", "mcore")
temporary = destination + ".tmp"
torch.save(state, temporary)
os.replace(temporary, destination)
PY
    printf '%s\n' release > "${compat_ref}/latest_checkpointed_iteration.txt"
    ref_load="${compat_ref}"
    log "Reference checkpoint compatibility view: ${ref_load} (async_strategy=mcore)"
}
...
PY
    ...
    if [ "${use_dynamic_batch_size}" = "1" ]; then
        batching_args=(--use-dynamic-batch-size --max-tokens-per-gpu "${max_tokens_per_gpu}")
    else
        batching_args=(--micro-batch-size "${micro_batch_size}")
    fi
    if [ "${reference_recipe}" != "1" ]; then
        batching_args=(--qkv-format "${qkv_format}" "${batching_args[@]}")
    fi

    local global_batch_args=()
    if [ "${reference_recipe}" = "1" ] || [ "${use_dynamic_batch_size}" != "1" ]; then
    ...
    local train_args=(
        ...
        --lr-decay-iters "${target_num_rollout:-5}"
