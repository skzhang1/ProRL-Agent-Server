#!/usr/bin/env bash
#SBATCH --job-name=webarea-debug-tmax-pi-16n-dapo-profile
...
# 16-node TMax PI DAPO profiling launcher.
#
# This is intentionally a short-run, env-overridable profiler: each submission
# runs one or two optimizer steps, records rollout/train timings, and exits.
set -euo pipefail
...
profile_id="${profile_id:-b32_s8_t2_r112_a4_gw128}"
run_id="${run_id:-webarea-debug_tmax_pi_16n_dapo_${profile_id}_${SLURM_JOB_ID}}"
run_label="${run_label:-16n-dapo-${profile_id}}"
...
export wandb_group="${wandb_group:-16n-dapo-profile}"
...
cat <<SUMMARY
============================================================
webarea TMax PI 16-node DAPO profile
  job_id:    ${SLURM_JOB_ID}
