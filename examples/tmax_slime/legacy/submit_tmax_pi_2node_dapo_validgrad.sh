#!/usr/bin/env bash
#SBATCH --job-name=webarea-debug-tmax-pi-2n-dapo-validgrad
...
# Two-node TMax PI DAPO-style valid-gradient smoke.
...
run_id="${run_id:-webarea-debug_tmax_pi_2n_dapo_validgrad_${SLURM_JOB_ID}}"
run_label="${run_label:-2n-tmax-dapo-validgrad}"
...
# Use a wider prompt pool so DAPO dynamic sampling can find non-degenerate groups.
export tmax_scan_tasks="${tmax_scan_tasks:-512}"
export tmax_only_ready="${tmax_only_ready:-1}"
export smoke_rows="${smoke_rows:-64}"
