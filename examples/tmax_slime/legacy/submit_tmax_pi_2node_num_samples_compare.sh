#!/usr/bin/env bash
#SBATCH --job-name=tmax-pi-num-samples-2n
...
# Independent two-node PI + Qwen3.5-4B num-samples comparison. It never
# trains or updates model weights. Each job runs two 8-group rollout steps
# with one gateway per node and 64 run workers per gateway.
set -euo pipefail
...
# Compare 16 and 32 samples under otherwise identical rollout settings.
gpus_per_node=8
num_nodes=2
total_gpus=16
train_num_gpus=0
...
polar_fully_async=1
polar_early_stop_grace_sessions="${polar_early_stop_grace_sessions:-$((n_samples_per_prompt / 4))}"
...
run_label="num-samples-${n_samples_per_prompt}-2step-w${polar_gateway_max_run_workers}"
experiment_name="${experiment_name:-tmax_pi_4b_num_samples_${n_samples_per_prompt}_2step_w${polar_gateway_max_run_workers}}"
...
case "${n_samples_per_prompt}" in
    16|32) ;;
    *) die "num-samples comparison requires n_samples_per_prompt=16 or 32" ;;
esac
expected_global_batch_size=$((rollout_batch_size * n_samples_per_prompt))
expected_grace_sessions=$((n_samples_per_prompt / 4))
early_stop_target=$((n_samples_per_prompt - polar_early_stop_grace_sessions))
[ "${global_batch_size}" = "${expected_global_batch_size}" ] || die "global_batch_size must equal rollout_batch_size * n_samples_per_prompt"
[ "${target_num_rollout}" = "2" ] || die "num-samples comparison requires exactly two rollout steps"
...
[ "${polar_early_stop_grace_sessions}" = "${expected_grace_sessions}" ] || die "early-stop grace must be 25% of n_samples_per_prompt"
[ "${polar_task_timeout_seconds}" = "900" ] || die "gateway sweep requires session timeout=900s"
[ "${polar_gateway_max_run_workers}" = "64" ] || die "num-samples comparison requires 64 run workers per gateway"
...
cat <<SUMMARY
============================================================
TMax PI 2-step num-samples comparison
  run:       ${run_id}
  nodes:     ${slurm_nodes[*]}
  ...
  mode:      debug-rollout-only, ${target_num_rollout} batches / ${global_batch_size} sessions each; early-stop target=${early_stop_target}/${n_samples_per_prompt}
