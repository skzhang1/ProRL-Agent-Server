#!/usr/bin/env bash
#SBATCH --job-name=tmax-pi-rollout8-2n
...
#SBATCH --output=/lustre/fs1/portfolios/llmservice/projects/llmservice_fm_vision/users/shaokunz/HarnessGen/ProRL-Agent-Server/examples/tmax_slime/tmp/rollout_8step_2node/%x-%j.out
#SBATCH --error=/lustre/fs1/portfolios/llmservice/projects/llmservice_fm_vision/users/shaokunz/HarnessGen/ProRL-Agent-Server/examples/tmax_slime/tmp/rollout_8step_2node/%x-%j.err
#SBATCH --export=ALL

# Independent two-node PI + Qwen3.5-4B rollout-only stability test. It never
# trains or updates model weights. Each step contains 8x32=256 trajectories;
# repeated four-hour interactive allocations resume one logical 8-step run.
set -euo pipefail
...
test_output_dir="${script_dir}/tmp/rollout_8step_2node"
...
target_num_rollout="${target_num_rollout:-8}"
...
run_label="rollout8-2n"
experiment_name="${experiment_name:-tmax_pi_4b_rollout8_2n}"
...
cat <<SUMMARY
============================================================
TMax PI 8-step rollout-only stability test
  run:       ${run_id}
  nodes:     ${slurm_nodes[*]}
  ...
  mode:      debug-rollout-only, ${target_num_rollout} batches / ${global_batch_size} sessions each
