#!/usr/bin/env bash
# Container-side launcher for TMax PI + Slime GRPO.
...
run_id="${run_id:-webarea-debug_tmax_pi_qwen35_4b_$(date -u +%Y%m%dT%H%M%SZ)}"
run_label="${run_label:-tmax-smoke}"
dry_run="${dry_run:-0}"
...
dep_root="${dep_root:-${project_root}/tmp/tmax_deps}"
...
tmax_scan_tasks="${tmax_scan_tasks:-256}"
tmax_only_ready="${tmax_only_ready:-1}"
tmax_train_start_index="${tmax_train_start_index:-0}"
tmax_train_data_pool="$(abs_path "${tmax_train_data_pool:-${run_dir}/tmax_ready_pool.jsonl}")"
full_prompt_data="$(abs_path "${full_prompt_data:-${tmax_train_data_pool}}")"
prompt_data="$(abs_path "${prompt_data:-${run_dir}/tmax_train_${smoke_rows}.jsonl}")"

apptainer_image_dir="$(abs_path "${apptainer_image_dir:-${tmax_image_dir}}")"
agent_cli_dir="$(abs_path "${agent_cli_dir:-${project_root}/tmp/tmax_agent_cli/opt_node}")"
...
job_cache_root="${job_cache_root:-/tmp/webarea-debug-tmax-${agent_label}-${SLURM_JOB_ID:-manual}-${run_id}}"
...
polar_request_timeout="${polar_request_timeout:-1800}"
...
wandb_project=harness-tma
wandb_group="${wandb_group:-webarea-debug-tmax-${agent_label}-qwen35-4b-${run_label}}"
...
prepare_prompt_data_and_sifs() {
    case "${tmax_only_ready}" in
        0|1) ;;
        *) die "tmax_only_ready must be 0 or 1; got ${tmax_only_ready}" ;;
    esac
    positive_int "${tmax_scan_tasks}" || die "tmax_scan_tasks must be a positive integer"
    case "${smoke_rows}" in
        ''|*[!0-9]*) die "smoke_rows must be a non-negative integer" ;;
    esac

    local prepare_args=(
        --dataset-dir "${tmax_dataset_dir}"
        --image-dir "${tmax_image_dir}"
        --output "${full_prompt_data}"
        --start-index "${tmax_train_start_index}"
        --max-tasks "${tmax_scan_tasks}"
    )
    if [ "${tmax_only_ready}" = "1" ]; then
        prepare_args+=(--only-ready)
    fi

    log "Preparing TMax prompt pool from dataset=${tmax_dataset_dir}, sif=${tmax_image_dir}"
    "${python_bin}" "${script_dir}/prepare_data.py" "${prepare_args[@]}"
...
if limit > 0:
    rows = rows[:limit]
if not rows:
    raise SystemExit(f"no TMax prompt rows selected from {pool}")
...
if missing:
    preview = "\n  ".join(missing[:20])
    raise SystemExit(f"invalid TMax selected rows:\n  {preview}")

prompt_out.parent.mkdir(parents=True, exist_ok=True)
prompt_out.write_text("\n".join(json.dumps(row, ensure_ascii=True) for row in rows) + "\n")
print(f"Prompt data: {prompt_out} ({len(rows)} TMax row(s) from pool {pool})")
print("Selected tasks:")
for row in rows:
    print(f"  {row['metadata']['task_name']} -> {row['metadata']['sif_path']}")
PY
}
...
if os.environ.get("POLAR_TASK_TIMEOUT_FROM_METADATA") == "1":
...
preflight() {
    ...
    [ -d "${tmax_dataset_dir}" ] || die "TMax dataset dir not found: ${tmax_dataset_dir}"
    [ -d "${tmax_image_dir}" ] || die "TMax SIF dir not found: ${tmax_image_dir}"
    [ -s "${prompt_data}" ] || die "TMax training data not found: ${prompt_data}"
...
log "TMax dataset: ${tmax_dataset_dir}"
log "TMax SIF source: ${tmax_image_dir}"
log "Prompt source: ${prompt_data}"
