#!/usr/bin/env bash
#SBATCH --job-name=tmax-pi-minimal-4n
...
# Minimal four-node PI + Qwen3.5-4B TMax GRPO training, based on the stable
# SWE-Gym launcher. One training node and three rollout nodes are unchanged.
...
# Stable TMax mode uses 1 prompt group x 8 samples = 8 samples/step.
gpus_per_node=8
num_nodes=4
total_gpus=32
train_num_gpus=8
...
num_epoch=1
# Slime treats --num-rollout as an exclusive boundary; checkpoint 200 requires 201.
target_num_rollout="${target_num_rollout:-201}"
...
polar_min_complete_accept_fraction="${polar_min_complete_accept_fraction:-0.5}"
...
polar_request_timeout="${polar_request_timeout:-900}"
...
run_label=${run_label:-tmax-minimal-4n-rb1-s8}
experiment_name=${experiment_name:-tmax_pi_minimal_4n_rb1_s8}
...
# Persist the effective per-session timeout across singleton allocations. A
# successor reduces it by 10 percent only when the previous allocation was
# cancelled by the occupied-idle-job reaper. Clean four-hour handoffs keep the
# same value, while unrelated failures remain unchanged for diagnosis.
polar_timeout_state_file="${run_dir}/polar_session_timeout_seconds.txt"
polar_timeout_job_file="${run_dir}/polar_session_timeout_last_job.txt"
polar_timeout_initial_seconds="${polar_timeout_initial_seconds:-840}"
polar_timeout_decay_percent="${polar_timeout_decay_percent:-90}"
case "${polar_timeout_initial_seconds}" in
    ''|*[!0-9]*) die "polar_timeout_initial_seconds must be a positive integer" ;;
esac
case "${polar_timeout_decay_percent}" in
    ''|*[!0-9]*) die "polar_timeout_decay_percent must be an integer in [1, 99]" ;;
esac
[ "${polar_timeout_initial_seconds}" -ge 1 ] || die "polar_timeout_initial_seconds must be positive"
[ "${polar_timeout_decay_percent}" -ge 1 ] && [ "${polar_timeout_decay_percent}" -le 99 ] || \
    die "polar_timeout_decay_percent must be in [1, 99]"

effective_session_timeout="${polar_timeout_initial_seconds}"
if [ -s "${polar_timeout_state_file}" ]; then
    effective_session_timeout="$(<"${polar_timeout_state_file}")"
fi
case "${effective_session_timeout}" in
    ''|*[!0-9]*) die "invalid session timeout state: ${effective_session_timeout}" ;;
esac

previous_timeout_job=""
if [ -s "${polar_timeout_job_file}" ]; then
    previous_timeout_job="$(<"${polar_timeout_job_file}")"
fi
timeout_change_reason="unchanged"
if [ -n "${previous_timeout_job}" ] && [ "${previous_timeout_job}" != "${SLURM_JOB_ID}" ]; then
    previous_job_record="$(scontrol show job -o "${previous_timeout_job}" 2>/dev/null || true)"
    case "${previous_job_record}" in
        *"AdminComment=Cancelled by occupied idle job reaper:"*)
            old_session_timeout="${effective_session_timeout}"
            effective_session_timeout=$(( (effective_session_timeout * polar_timeout_decay_percent + 50) / 100 ))
            [ "${effective_session_timeout}" -ge 1 ] || effective_session_timeout=1
            timeout_change_reason="idle-reaper job ${previous_timeout_job}: ${old_session_timeout}s -> ${effective_session_timeout}s"
            ;;
    esac
fi
printf '%s\n' "${effective_session_timeout}" > "${polar_timeout_state_file}"
printf '%s\n' "${SLURM_JOB_ID}" > "${polar_timeout_job_file}"
polar_task_timeout_from_metadata=0
polar_task_timeout_seconds="${effective_session_timeout}"
printf 'Polar per-session timeout: %ss (%s)\n' "${effective_session_timeout}" "${timeout_change_reason}"
...
export POLAR_APPTAINER_BIN POLAR_APPTAINER_DIRECT_EXEC POLAR_APPTAINER_EXEC_MODE
...
strace -ff -tt -T -e signal=all \
    -e trace=kill,tkill,tgkill,rt_sigqueueinfo,rt_tgsigqueueinfo \
    -o "${run_log_dir}/srun-signal.strace" \
    srun --mpi=none --overlap --kill-on-bad-exit=1 --nodes="${num_nodes}" --ntasks="${num_nodes}" --ntasks-per-node=1 --gres="gpu:${gpus_per_node}" \
        ...
        bash "${worker_script}" &
srun_wrapper_pid="$!"
printf '[%s] srun strace wrapper started pid=%s\n' \
    "$(date -u +%FT%TZ)" "${srun_wrapper_pid}" >>"${batch_monitor_log}"

(
    while true; do
        printf 'timestamp=%s host=%s monitor_pid=%s batch_pid=%s srun_wrapper_pid=%s\n' \
            "$(date -u +%FT%TZ)" "$(hostname)" "${BASHPID}" "$$" "${srun_wrapper_pid}"
        printf '%s\n' 'self cgroup:'
        ...
        ps -o pid,ppid,state,rss,vsz,etimes,comm,args --forest \
            -p "$$,${srun_wrapper_pid}" --ppid "$$,${srun_wrapper_pid}" 2>/dev/null || true
        sleep 10
    done
) >>"${batch_monitor_log}" 2>&1 &
batch_monitor_pid="$!"

wait "${srun_wrapper_pid}"
srun_rc="$?"
set -e
