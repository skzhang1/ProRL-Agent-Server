#!/usr/bin/env bash
#SBATCH --job-name=tmax-pi-traces300-2n
...
#SBATCH --output=/lustre/fs1/portfolios/llmservice/projects/llmservice_fm_vision/users/shaokunz/HarnessGen/ProRL-Agent-Server/examples/tmax_slime/tmp/trajectory_rollout_300/%x-%j.out
#SBATCH --error=/lustre/fs1/portfolios/llmservice/projects/llmservice_fm_vision/users/shaokunz/HarnessGen/ProRL-Agent-Server/examples/tmax_slime/tmp/trajectory_rollout_300/%x-%j.err
#SBATCH --export=ALL

# Independent two-node PI + Qwen3.5-4B trajectory rollout. It never trains or
# updates model weights and never patches Polar, Slime, Megatron, or SGLang.
# Two validated 8x32 batches provide enough candidates for exactly 300 traces.
set -euo pipefail
...
test_output_dir="${script_dir}/tmp/trajectory_rollout_300"
inner_launcher="${script_dir}/run_tmax_apptainer_rollout_nopatch.sh"
collector_script="${script_dir}/collect_sglang_node_metrics.py"
aggregate_script="${script_dir}/aggregate_trajectory_rollout.py"
...
# Match the target DPPO rollout load: 8 prompt groups x 32 samples.
gpus_per_node=8
num_nodes=2
total_gpus=16
train_num_gpus=0
...
num_epoch=1
# Slime treats --num-rollout as an exclusive boundary: IDs 0..7 require 8.
...
polar_gateway_max_run_workers="${polar_gateway_max_run_workers:-64}"
...
run_label="traces300-2n"
experiment_name="${experiment_name:-tmax_pi_4b_traces300_2n_nopatch}"
...
metrics_csv="${metrics_csv:-${run_dir}/node_token_usage_5s.csv}"
summary_json="${summary_json:-${script_dir}/tmax_pi_2node_300_agent_traces.json}"
...
[ -f "${collector_script}" ] || die "missing metrics collector: ${collector_script}"
[ -f "${aggregate_script}" ] || die "missing trajectory aggregator: ${aggregate_script}"
...
export project_root script_dir inner_launcher collector_script aggregate_script train_sqsh container_mounts slime_dir megatron_dir
...
export run_id run_label run_dir run_log_dir save_dir rollout_save_dir metrics_csv summary_json
export ref_load
...
export ray_head_ip sglang_router_host stop_file head_node worker_node worker_node_ip
...
cleanup() {
    ...
    [ -z "${token_monitor_pid}" ] || kill "${token_monitor_pid}" 2>/dev/null || true
...
if [ "${rank}" = "0" ]; then
    ...
    python "${collector_script}" \
        --router-url "http://${sglang_router_host}:${sglang_router_port}" \
        --node-map "${head_node}=${ray_head_ip}" \
        --node-map "${worker_node}=${worker_node_ip}" \
        --interval-seconds 5 \
        --output "${metrics_csv}" \
        >>"${run_log_dir}/node-token-collector.log" 2>&1 &
    token_monitor_pid="$!"
...
cat <<SUMMARY
============================================================
TMax PI 300-trace rollout-only job
  run:       ${run_id}
  nodes:     ${slurm_nodes[*]}
  ...
  mode:      debug-rollout-only, no runtime patch, ${target_num_rollout} batches / ${global_batch_size} sessions each
